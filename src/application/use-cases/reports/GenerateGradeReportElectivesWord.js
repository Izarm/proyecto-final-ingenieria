...
class GenerateGradeReportElectivesWord {
    ...
    async execute({ academicYearId, periodId, gradeId }) {
        ...
        // Electivas: group_id IS NULL (toda la institución)
        const [subjects] = await this.pool.query(
            `SELECT DISTINCT sa.id, s.id as subject_id, s.name
             ...
             WHERE sa.academic_year_id = ? AND sa.is_elective = 1 AND sa.deleted_at IS NULL
             ORDER BY s.name`, [academicYearId]);
        ...
        if (studentIds.length && subjects.length) {
            const [rows] = await this.pool.query(
                `SELECT gr.normal_note, e.student_id, sa.subject_id
                 ...
                 WHERE e.student_id IN (?) AND gr.period_id = ? AND sa.is_elective = 1 AND gr.deleted_at IS NULL`,
                [studentIds, periodId]);
            rows.forEach(r => {
        ...
        const doc = new Document({ sections: [{
            ...
            children: [
                ...
                new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 }, children: [new TextRun({ text: 'INFORME NOTAS ACTIVIDADES FORMATIVAS (ELECTIVAS)', bold: true, size: 22, font: 'Arial' })] }),
...
module.exports = GenerateGradeReportElectivesWord;
